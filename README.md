...
- **stdio**: Standard input/output (default, for desktop clients like Claude for Desktop)
- **sse**: HTTP Server-Sent Events (for web-based clients and remote connections)
...
This will start the server in stdio mode and allow you to test the available tools.

#### HTTP/SSE Transport

To run the server with HTTP streaming support:
...
uv run server.py --transport sse --host 127.0.0.1 --port 3000
```

The SSE endpoint will be available at `http://<host>:<port>/sse`

**Command-line options:**
- `--transport`: Choose between `stdio` (default) or `sse`
- `--host`: Host address to bind the SSE server (default: `0.0.0.0`)
- `--port`: Port number for the SSE server (default: `8000`)

Example connecting to the SSE endpoint:
```bash
...
#### Using SSE Transport with Claude for Desktop

If you prefer to run the server as a standalone HTTP service:

1. Start the server with SSE transport:
   ```bash
   uv run server.py --transport sse --host 127.0.0.1 --port 8000
   ```

2. Configure Claude for Desktop to connect via HTTP:
   ```json
   {
     "mcpServers": {
       "yfinance": {
